...
clientname = "IOT_PROJECT"
relay_topic = 'Smart_Lock_Relay'
global ON
# ON = False
conn = sqlite3.connect('doors.db')
c=conn.cursor()
c.execute("SELECT * FROM doorsTable WHERE doorNumber=:doorNumber", {'doorNumber': 21})
ON = c.fetchone()[1]
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        text="doors on floor 21"
        self.ePushtbtn = QPushButton(text, self)
    ...
    def update_btn_state(self, text):
        ...
        if ON == "open":
            self.ePushtbtn.setStyleSheet("background-color: gray")
            self.mc.publish_to("Smart_Lock","closes the doors on the 21st floor ")
            ON= "close"
        else:
            self.ePushtbtn.setStyleSheet("background-color: red")
            self.mc.publish_to("Smart_Lock","Opens the doors on the 21st floor ")
            ON= "open"
